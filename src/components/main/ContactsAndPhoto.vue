  ...
  <div class="main--section main--section--contacts-and-photo px-4 md:px-0">
    <div class="main--section--feedback md:max-w-[1180px] justify-between flex-col md:flex-row md:gap-14 w-screen px-4 md:px-0">
      ...
      <div class="main--section--photo flex flex-col w-[100%] md:w-[480px]">
        ...
        <span @input="changeNameInput" :contenteditable="true" class="input" :class="{'text-[#000]': name, 'text-[#BBBBBB] hintable': !name}">{{name ?? 'Имя Фамилия'}}</span>
        <span @input="changeAddressInput" :contenteditable="true" class="input" :class="{'text-[#000]': address, 'text-[#BBBBBB] hintable': !address}">{{address ?? 'Откуда вы (населенный пункт)'}}</span>
